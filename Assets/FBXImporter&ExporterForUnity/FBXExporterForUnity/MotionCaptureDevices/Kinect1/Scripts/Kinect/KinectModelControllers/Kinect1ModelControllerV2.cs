/*
 * Kinect1ModelControllerV2.cs - Handles rotating the bones of a model to match 
 * 			rotations derived from the bone positions given by the kinect1
 * 
 * 		Developed by Peter Kinney -- 2011/06/30
 *  	Modified by ほえたん(Hoetan) -- 2015/09/01
...
public class Kinect1ModelControllerV2 : MonoBehaviour {
	...
	public float rotationWeight = 0.2f;

	public bool bFixedUpdateMode = false;

	private bool bPollSkeleton = false;
	...
	void Start () {
		...
		_vecbones = new Vector3[25];
		_vecbones2 = new Vector3[25];
		_qbones = new Quaternion[25];
		_qbones2 = new Quaternion[25];
		...
		_baseRotation = new Quaternion[25];
	...
	void RotateJoint(int bone) {
		...
		if(bone == (int)Kinect.NuiSkeletonPositionIndex.Spine)
		{
			...
			_vecbones[bone] = sw.bonePos[player,(int)Kinect.NuiSkeletonPositionIndex.HipCenter];
		}
	...
	void RotateJointUpdate(int bone)
	{
		if (_bones[bone] != null) {
			if(bone == (int)Kinect.NuiSkeletonPositionIndex.Spine)
			{
				Vector3 vecPos = Vector3.Lerp(_vecbones2[bone], _vecbones[bone], moveWeight);
			...
			Quaternion quat = Quaternion.Lerp (_qbones2[bone], _qbones[bone], rotationWeight);
